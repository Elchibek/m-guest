...
/**
 * Performance test for the PutAway entity.
 */
public class PutAwayGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all putAways").get("/api/put-aways").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new putAway")
                        .post("/api/put-aways")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"countPerson\": 0" +
                                ", \"whenLeft\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"isAll\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_putAway_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created putAway").get("${new_putAway_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created putAway").delete("${new_putAway_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PutAway entity").exec(scn);
