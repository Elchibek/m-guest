 ...
 */
public class FloorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all floors").get("/api/floors").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new floor")
                        .post("/api/floors")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"numFloor\": 0" + ", \"backgroundColor\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_floor_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created floor").get("${new_floor_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created floor").delete("${new_floor_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Floor entity").exec(scn);
