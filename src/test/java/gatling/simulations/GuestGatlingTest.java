...
/**
 * Performance test for the Guest entity.
 */
public class GuestGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all guests").get("/api/guests").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new guest")
                        .post("/api/guests")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"guestInstitution\": \"SAMPLE_TEXT\"" +
                                ", \"responsible\": \"SAMPLE_TEXT\"" +
                                ", \"isArchive\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_guest_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created guest").get("${new_guest_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created guest").delete("${new_guest_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Guest entity").exec(scn);
