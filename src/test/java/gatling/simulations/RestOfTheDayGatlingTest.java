...
/**
 * Performance test for the RestOfTheDay entity.
 */
public class RestOfTheDayGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all restOfTheDays").get("/api/rest-of-the-days").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new restOfTheDay")
                        .post("/api/rest-of-the-days")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"willStay\": 0" + ", \"num\": 0" + ", \"isDeparture\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_restOfTheDay_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created restOfTheDay").get("${new_restOfTheDay_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created restOfTheDay").delete("${new_restOfTheDay_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the RestOfTheDay entity").exec(scn);
