...
/**
 * Performance test for the GuestFrom entity.
 */
public class GuestFromGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all guestFroms").get("/api/guest-froms").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new guestFrom")
                        .post("/api/guest-froms")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_guestFrom_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created guestFrom").get("${new_guestFrom_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created guestFrom").delete("${new_guestFrom_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GuestFrom entity").exec(scn);
