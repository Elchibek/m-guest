...
/**
 * Performance test for the DidntPay entity.
 */
public class DidntPayGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all didntPays").get("/api/didnt-pays").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new didntPay")
                        .post("/api/didnt-pays")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"countPerson\": 0" + ", \"paid\": 0" + ", \"explanation\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_didntPay_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created didntPay").get("${new_didntPay_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created didntPay").delete("${new_didntPay_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DidntPay entity").exec(scn);
