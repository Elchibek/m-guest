...
/**
 * Performance test for the GuestContact entity.
 */
public class GuestContactGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all guestContacts").get("/api/guest-contacts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new guestContact")
                        .post("/api/guest-contacts")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_guestContact_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created guestContact").get("${new_guestContact_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created guestContact").delete("${new_guestContact_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GuestContact entity").exec(scn);
