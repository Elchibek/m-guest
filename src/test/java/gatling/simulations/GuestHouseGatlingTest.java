...
/**
 * Performance test for the GuestHouse entity.
 */
public class GuestHouseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all guestHouses").get("/api/guest-houses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new guestHouse")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"isEmpty\": null" +
                                ", \"countPerson\": 0" +
                                ", \"backgroundColor\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_guestHouse_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created guestHouse").get("${new_guestHouse_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created guestHouse").delete("${new_guestHouse_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GuestHouse entity").exec(scn);
