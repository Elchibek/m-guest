...
/**
 * Performance test for the GuestStatic entity.
 */
public class GuestStaticGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all guestStatics").get("/api/guest-statics").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new guestStatic")
                        .post("/api/guest-statics")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"totalDidntPay\": 0" +
                                ", \"numOfApartments\": 0" +
                                ", \"affordableApartments\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_guestStatic_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created guestStatic").get("${new_guestStatic_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created guestStatic").delete("${new_guestStatic_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GuestStatic entity").exec(scn);
