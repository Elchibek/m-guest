...
/**
 * Performance test for the ArrivalDepartureStatic entity.
 */
public class ArrivalDepartureStaticGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all arrivalDepartureStatics")
                    .get("/api/arrival-departure-statics")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new arrivalDepartureStatic")
                        .post("/api/arrival-departure-statics")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"isArrival\": null" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + ", \"countPerson\": 0" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_arrivalDepartureStatic_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created arrivalDepartureStatic")
                            .get("${new_arrivalDepartureStatic_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created arrivalDepartureStatic")
                        .delete("${new_arrivalDepartureStatic_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ArrivalDepartureStatic entity").exec(scn);
